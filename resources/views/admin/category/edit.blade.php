...
@section('content')
    ...
    <div class="container-fluid mt-4">
        ...
        <form class="mt-4" action="{{ route('admin.categories.edit', ['category'=> $category]) }}" method="POST">
            @csrf
            ...
            <div class="row">
                <div class="form-group col-md-6 offset-md-3">
                    ...
                    <div class="d-flex justify-content-center">
                        <p id="icon-p" class="text-center">{!! old('icon')??$category->icon !!}</p>
                    </div>
                   <input type="text" name="icon" value="{{ old('icon')??$category->icon }}" id="icon" required />
                   @error('icon')
            ...
            <div class="row">
                <div class="form-group col-md-6 offset-md-3">
                    ...
                    <input type="number" name="sort" id="sort" value="{{ old('sort')??$category->sort }}" class="form-control" placeholder="Category Sort" required>
                    @error('sort')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
