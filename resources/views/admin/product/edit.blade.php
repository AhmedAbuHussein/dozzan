        ...
        <form action="{{ route('admin.products.edit', ['product'=> $product->id]) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="form-group col-md-6 offset-md-3">
                    ...
                    <input type="text" name="name[en]" id="nameen" value="{{ old('name.en')??$product->name['en'] }}" class="form-control" placeholder="Item Name English" required>
                    @error('name.en')
                ...
                <div class="form-group col-md-6 offset-md-3">
                    ...
                    <input type="text" name="name[ar]" id="namear" value="{{ old('name.ar')??$product->name['ar'] }}" class="form-control" placeholder="Item Name Arabic" required>
                    @error('name.ar')
                    <span class="text-danger">
                        <strong>{{ $message }}</strong>
                    </span>
                ...
                <div class="form-group col-md-6 offset-md-3">
                    ...
                    <textarea rows="3" name="details[en]" id="detailsen" class="form-control" placeholder="Item details English" required>{{ old('details.en')??$product->details['en'] }}</textarea>
                    @error('details.en')
                    <span class="text-danger">
                        <strong>{{ $message }}</strong>
                    </span>
                ...
                <div class="form-group col-md-6 offset-md-3">
                    ...
                    <textarea rows="3" name="details[ar]" id="detailsar" class="form-control" placeholder="Item details Arabic" required>{{ old('details.ar')??$product->details['ar'] }}</textarea>
                    @error('details.ar')
                    <span class="text-danger">
                        <strong>{{ $message }}</strong>
                    </span>
                ...
                <div class="form-group col-md-6 offset-md-3">
                    ...
                    <select class="form-control" name="category_id" required>
                        ...
                        <option {{ ($product->category_id == $category->id || old('category_id') == $category->id)? 'selected':'' }} value="{{ $category->id }}">{{ $category->name_lang }}</option>
                        @endforeach
                    </select>
                    @error('category_id')
                    <span class="text-danger">
